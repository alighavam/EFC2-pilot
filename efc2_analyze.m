function varargout=efc2_analyze(what, varargin)
...
my_font.conf_legend = 20;
my_font.conf_title = 36;

% conference fig sizes:
conf.err_width = 3;
conf.line_width = 8;
...
switch (what)
    case 'subject_routine'
        ...
        vararginoptions(varargin,{'subject_num','smoothing_win_length'});
        
        % if a cell containing multiple subjects was given:
        if (iscell(subject_num))
            for i = 1:length(subject_num)
                efc2_subj(subject_num{i},'smoothing_win_length',smoothing_win_length)
            end
        % if a single subject as a char was given:
        else
            efc2_subj(subject_num,'smoothing_win_length',smoothing_win_length);
        end
    ...
    case 'make_all_dataframe'
        ...
        for k = 1:length(day)
            ...
            files = dir(fullfile(project_path, 'analysis', ['efc2_*' '_day_' num2str(k) '_raw.tsv']));
            movFiles = dir(fullfile(project_path, 'analysis', ['efc2_*' '_day_' num2str(k) '_mov.mat']));
            ...
            for i = 1:length({files(:).name})
                ...
                % is testing day:
                tmp_data.is_testing_day = ismember(day(k),[1,5]) * ones(size(tmp_data.force_e1));
                
                % adding subject data to ANA:
                ANA=addstruct(ANA,tmp_data,'row','force');
            end
        
            % dsave(fullfile(project_path,'analysis',['efc2_all_' 'day_' num2str(k) '.tsv']),ANA);
        end
        ANA.trained_chord = ismember(ANA.chordID,trained);
        ANA.repetition = kron(ones(length(ANA.chordID)/5,1),(1:5)');
        
        dsave(fullfile(project_path,'analysis','efc2_all.tsv'),ANA);
    ...
    case 'repetition_effect'        
        data = dload(fullfile(project_path, 'analysis', 'efc2_all.tsv'));

        repetitions = 5;
        df = [];
        for i = 1:repetitions
            % trained chords:
            row = data.repetition==i & data.trained_chord==1 & data.trialCorr==1; 
            [~, day, RT, ~, SN] = get_sem(data.RT(row), data.sn(row), data.day(row), ones(sum(row),1));
            [~, day, MT, ~, SN] = get_sem(data.MT(row), data.sn(row), data.day(row), ones(sum(row),1));
            [~, day, MD, ~, SN] = get_sem(data.MD(row), data.sn(row), data.day(row), ones(sum(row),1));
            tmp = [];
            tmp.sn = SN;
            tmp.day = day;
            tmp.repetition = i*ones(length(day),1);
            tmp.trained_chord = ones(length(day),1);
            tmp.RT = RT;
            tmp.MT = MT;
            tmp.MD = MD;
            df = addstruct(df,tmp,'row','force');

            % untrained chords:
            row = data.repetition==i & data.trained_chord==0 & data.trialCorr==1; 
            [~, day, RT, ~, SN] = get_sem(data.RT(row), data.sn(row), data.day(row), ones(sum(row),1));
            [~, day, MT, ~, SN] = get_sem(data.MT(row), data.sn(row), data.day(row), ones(sum(row),1));
            [~, day, MD, ~, SN] = get_sem(data.MD(row), data.sn(row), data.day(row), ones(sum(row),1));
            tmp = [];
            tmp.sn = SN;
            tmp.day = day;
            tmp.repetition = i*ones(sum(row),1);
            tmp.trained_chord = zeros(length(day),1);
            tmp.RT = RT;
            tmp.MT = MT;
            tmp.MD = MD;
            df = addstruct(df,tmp,'row','force');
        ...
        trained_chord = reshape(data.trained_chord,5,[]); 
        day = reshape(data.day,5,[]); 
        MD = reshape(MD,5,[]);
        RT = reshape(RT,5,[]);
        ...
        % getting averages within each day:
        day = day(1,:);
        subj = subj(1,:);
        ...
        for i = 1:1
            ...
            for j = 1:length(unique(day))
                for sn = 1:length(subj_unique)
                    ...
                    % subject:
                    MD_tmp = MD(:, subj==subj_unique(sn) & day==j);
                    ...
                    RT_tmp = RT(:, subj==subj_unique(sn) & day==j);
